...
start_monitoring() {
    ...
    if command -v vmstat &> /dev/null; then
        {
            LC_ALL=C stdbuf -oL -eL vmstat -n $SYSTEM_SAMPLE_SECS \
    ...
    if command -v iostat &> /dev/null; then
        ...
        (
            LC_ALL=C stdbuf -oL -eL iostat -c -y "$SYSTEM_SAMPLE_SECS" \
            | awk '
        ...
        add_monitoring_pid $! "iostat_cpu"

        # --- DEVICE CSV (iostat -xd -y 1 -k) ---
        (
            LC_ALL=C stdbuf -oL -eL iostat -xd -y -k "$SYSTEM_SAMPLE_SECS" \
            | awk '
    ...
    (
        LC_ALL=C stdbuf -oL -eL free -m -s "$SYSTEM_SAMPLE_SECS" \
